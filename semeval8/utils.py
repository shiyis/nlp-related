...
class SemEvalDateset(Dataset):
    ...
    def __symbolize_sentence(self, sentence):
        ...
        mask = [1] * len(sentence)
        words = []
        valid_length = min(self.max_len, len(sentence))
        ...
        if valid_length < self.max_len:
            for i in range(valid_length, self.max_len):
                mask.append(0)  # 'PAD' mask is zero
                words.append(self.word2id['PAD'])

        unit = np.asarray([words, mask, [valid_length]], dtype=np.int64)
...
class SemEvalDataLoader(object):
    ...
    def _collate_fn(self, batch):
        ...
        data = list(data)
        label = list(label)
        data = mx.nd.array(np.concatenate(data, axis=0))
        label = mx.nd.array(label, dtype=np.int64)
        return data, label

    def _get_data(self, filename, shuffle=False):
    ...
    def get_train(self):
        return self._get_data('train.json', shuffle=True)

    def get_dev(self):
        return self._get_data('test.json', shuffle=False)
